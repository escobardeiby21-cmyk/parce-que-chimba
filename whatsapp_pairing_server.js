  ...
  if (session.lastMenu === 'confirming') {
    ...
    if (!t.includes('hamburguesa') && !t.includes('perro') && !t.includes('salchipapa') && !t.includes('picada') && !t.includes('alita') && !t.includes('menu')) {
      ...
      const newOrder = {
        ...
        items: session.cart.length > 0 ? session.cart : [{ name: 'Pedido por WhatsApp AI', price: session.total > 0 ? session.total : 0.00, quantity: 1 }],
        subtotal: session.total > 0 ? session.total : (session.cart.length > 0 ? session.cart.reduce((s, i) => s + ((i.price || 0) * (i.quantity || 1)), 0) : 0.00),
        deliveryFee: 2.00,
        total: (session.total > 0 ? session.total : (session.cart.length > 0 ? session.cart.reduce((s, i) => s + ((i.price || 0) * (i.quantity || 1)), 0) : 0.00)) + 2.00,
  ...
  try {
    ...
    if (msg.type === 'location') {
      ...
      const newOrder = {
        ...
        items: session.cart.length > 0 ? session.cart : [{ name: 'Pedido por WhatsApp GPS', price: session.total > 0 ? session.total : 0.00, quantity: 1 }],
        subtotal: session.total > 0 ? session.total : (session.cart.length > 0 ? session.cart.reduce((s, i) => s + ((i.price || 0) * (i.quantity || 1)), 0) : 0.00),
        deliveryFee: 2.00,
        total: (session.total > 0 ? session.total : (session.cart.length > 0 ? session.cart.reduce((s, i) => s + ((i.price || 0) * (i.quantity || 1)), 0) : 0.00)) + 2.00,
        status: 'En Preparación',
